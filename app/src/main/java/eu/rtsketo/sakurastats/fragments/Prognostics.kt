...
class Prognostics : Fragment() {
    ...
    private var loadingOp: ImageView? = null
    var wifi: ImageView? = null
        get() {
            if (field == null) field = root!!.findViewById(R.id.wifi)
            if (wifiOp == null) wifiOp = root!!.findViewById(R.id.wifiOp)
            return Pair(field, wifiOp)
        }
        private set
    private var wifiOp: ImageView? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val bgWaves = root.findViewById<ImageView>(R.id.bgWaves)
        (bgWaves.drawable as AnimationDrawable).start()
        Console.Companion.create(acti, console)
        loadingAnim.setOnClickListener(View.OnClickListener { v: View? -> refresh(true) })
        return root
    }
