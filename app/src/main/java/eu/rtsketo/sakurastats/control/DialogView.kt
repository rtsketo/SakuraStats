...
class DialogView(type: SakuraDialog, prevDia: Dialog?, count: Int, acti: Interface) {
    enum class SakuraDialog { INPUT, INFO, CLANQUEST, RATEQUEST }
    ...
    init {
        val inputView =
                if (type == SakuraDialog.INPUT) LayoutInflater.from(acti)
                        .inflate(R.layout.dialog_input, null)
                else LayoutInflater.from(acti)
        ...
        val size = intArrayOf(SDPMap.sdp2px(9), SDPMap.sdp2px(8))
        when (type) {
            SakuraDialog.INPUT -> {
                ...
                cancel.setOnClickListener { view: View ->
                    ...
                    else DialogView(SakuraDialog.CLANQUEST, dialog, acti)
                }
            }
                ...
                confirm.setOnClickListener { view: View ->
                    ...
                    try {
                        ...
                        ContextCompat.startActivity(acti, Intent(Intent.ACTION_VIEW,
                                Uri.parse("http://play.google.com/store/apps/detailsid="
                                        + acti.packageName)), null)
                    }
                    dialog.cancel()
                }
            }
